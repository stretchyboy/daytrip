...
parser.addArgument(  [ '-j', '--journeycalls' ],
  { help: "Make any Journey planning calls", action: 'storeTrue', defaultValue: true});
...
var aTooFar = [];
// TODO : options.postcode.replace(" ", "_")+"_"+options.radius
var sTooFarFile = ("data/too_far_"+safeBaseName(options.postcode)+".json");
...
aProperties = aProperties.filter(function(oProperty){
   ...
   //Filter out the Properties we that are Too Far
...
aProperties.forEach(function(oProperty) {
  if (options.debug) {console.log("Looking for Property", oProperty.search);}
...
function onLocation(oLocation, oProperty){
  var iDist = distance(oPostCode.latitude, oPostCode.longitude, oLocation.latitude, oLocation.longitude, "M");
            
  //console.log("iDist =", iDist);
  if(iDist < options.radius) {
    if(options.journeycalls && iJCalls < options.maxjcalls) {
      iJCalls ++;
    ...
    console.log(oProperty.search+" is too far at ", Math.round(iDist)+ " miles");
